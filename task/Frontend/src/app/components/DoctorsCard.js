...
export default function DoctorsCard({ doctor }) {
    return (
      <div className="bg-slate-200 p-4 rounded-lg shadow-sm border flex flex-col sm:flex-row gap-4">
  
        {/* Doctor Info */}
        <div className="flex-1">
          <h2 className="text-lg font-bold text-gray-900">{doctor?.name || "Unknown Doctor"}</h2>
          <p className="text-sm text-gray-800">{doctor?.specialization || "N/A"}</p>
        ...
        <div className="flex flex-col justify-between text-right sm:text-left">
          ...
          <button className="mt-2 bg-white text-blue-600 border border-blue-600 px-4 py-2 rounded hover:bg-blue-50 cursor-pointer">
            Consult Online
          </button>
        </div>
      </div>
